...
| S2b | ~~**Quick Selection retargeting**~~ **DONE 2026-07-07** | Magic Wand should produce a reusable selection, not paint directly. | S | Shipped: `SelectionShape::Mask{width,height,data}` — a raw per-texel coverage mask, for the one shape (flood-fill) with no compact vertex/analytic form. `Renderer::magic_wand_select` (was `paint_magic_wand_fill`) returns it instead of mutating pixels; `rasterize_selection_mask`/`selection_shape_bounds`/Move all gained a `Mask` arm. Flood-fill BFS extracted into a pure `flood_fill_mask` fn (CPU-testable, no GPU device needed) per the project's existing pure-fn-for-pixel-math discipline. **Caught pre-ship:** `handle_left_release`'s Lasso-cancel check used `lasso_points.len() < 3` as a proxy for "was this a Lasso drag" — true for every non-Lasso selection tool, so it silently cancelled every Ellipse (and would have cancelled every new Mask) selection right after mouse-up. Fixed by checking the shape variant itself instead of the side-channel counter. **Still open:** marching-ants for a Mask selection draws its bounding box, not the exact flood-filled outline (documented in the inspector, not silently wrong). |
| S2c | ~~**Selection feathering**~~ **DONE 2026-07-07** | Soft selection edges — expected of any real selection toolkit. | S | Shipped: a "Feather" slider (0–64px) in the selection inspector. Pure `feather_mask` (a tent = two edge-clamped separable box-blur passes, O(w·h)) applied to the rasterized mask; all five tool call sites route through one `Renderer::current_selection_mask` chokepoint, so Paint/Gradient/Move/Text all pick it up. `feather=0` is byte-for-byte the old hard-edge path. Fully CPU-unit-tested (no GPU/scissor change). v1 is inset-style (softens the interior side; doesn't grow the selection's bounds — a true both-directions feather needs per-tool region + scissor expansion, deferred). See DECISIONS.md. |
| S3 | ~~**Layer masks**~~ **DONE 2026-07-07** | The core of non-destructive editing. | M | Shipped: `PaintLayer.mask: Option<RasterCanvas>`; the composite shader multiplies each layer's alpha by its mask's red (`@binding(4)`, 1×1-white fallback → unmasked path byte-for-byte identical). **Verified on real hardware** by a headless-GPU test running the exact `LAYER_COMPOSITE_WGSL` (black hides / white reveals / all-white == no-op). Full authoring: **"From Sel"** (any selection tool + feather) · **"Add"** (blank white mask) · **paint directly** (Paint Mask toggle → brush black to hide, white to reveal, routed through the tested brush/undo path) · **Clear** · and it **saves/loads** in `.sweet` (sibling `mask.png` blob, back-compat with old projects; round-trip tested). Remaining polish: a panel thumbnail. See DECISIONS.md 2026-07-07 (both the core entry and the follow-ups entry). |
...
Tier 1  (selections) → mask texture (done) → lasso/poly (done) → wand→selection (done) → feather (done) → layer masks (done: create/paint/save) — Tier 1 complete
