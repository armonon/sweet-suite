...
| S3 | **Layer masks** | The core of non-destructive editing. | M | Reuse the S1 mask machinery per layer. Still the big open Tier 1 item — gated on a live-verifiable compositor change (see DECISIONS.md 2026-07-07 S3-deferral). |
...
Tier 1  (selections) → mask texture (done) → lasso/poly (done) → wand→selection (done) → feather (done) → layer masks
