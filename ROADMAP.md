...
| S3 | ~~**Layer masks**~~ **DONE (v1) 2026-07-07** | The core of non-destructive editing. | M | Shipped: `PaintLayer.mask: Option<RasterCanvas>`; the composite shader multiplies each layer's alpha by its mask's red (`@binding(4)`, 1×1-white fallback → unmasked path byte-for-byte identical). v1 authoring = **"Mask from Selection"** (reuses every selection tool + feathering) + Clear. **Verified on real hardware** by a headless-GPU test that runs the exact `LAYER_COMPOSITE_WGSL` and reads back — black mask hides / white reveals / all-white == no-op. **Deferred (shown in the panel):** painting directly on the mask, and mask persistence in `.sweet` (session-only for now). See DECISIONS.md 2026-07-07. |
...
Tier 1  (selections) → mask texture (done) → lasso/poly (done) → wand→selection (done) → feather (done) → layer masks (done, v1) — Tier 1 core complete
